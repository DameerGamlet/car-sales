...
public class CarModel : PageModel {
  ...
  public CarModel(ILogger<CarModel> _logger, ApplicationDbContext _context,
                  ICars _cars) {
    log = _logger;
    context = _context;
    cars = _cars;
  ...
  public IActionResult OnGetBrand([FromQuery] string brandId) {
    if (!int.TryParse(brandId, out int brandIdInt)) {
      // Обработка неверного формата brandId, например, возврат ошибки 400 Bad
      // Request.
  ...
  public IActionResult OnGetByBrand(int brandId) {
    log.LogInformation("Получение списка автомобилей для бренда: {brandId}",
                       brandId);
